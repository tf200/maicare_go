...
func (processor *AsynqServer) ProcessIncidentTask(ctx context.Context, t *asynq.Task) error {
	...
	err = processor.brevoConf.SendIncident(ctx, p.Emails, email.Incident{
	...
	if err != nil {
		...
		return fmt.Errorf("failed to send incident email to %v: %v: %w", p.Emails, err, asynq.SkipRetry)
	}

	return nil
}

func (processor *AsynqServer) ProcessIncidentConfirmedEmailTask(ctx context.Context, t *asynq.Task) error {
	var p aclient.IncidentConfirmedEmailPayload
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
	...
	err = processor.brevoConf.SendIncidentWithAttachment(ctx, recipients, email.Incident{
...
func (c *AsynqServer) ProcessClientCareStatusSyncTask(ctx context.Context, t *asynq.Task) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to activate due scheduled-out-of-care clients: %v: %w", err, asynq.SkipRetry)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("failed to list due scheduled-out-of-care clients missing final evaluation: %v: %w", err, asynq.SkipRetry)
	}
