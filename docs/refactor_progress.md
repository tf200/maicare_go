...
Status: scaffolded, unwired
Notes: migrated role/permission endpoints into the internal stack. Direct per-user permission overrides were subsequently removed; roles are now the sole permission source. Current endpoints cover role listing/creation, scoped role-permission replacement, permission listing, employee role assignment, and role-derived employee permissions. `ListAllPermissions` groups permissions by `GroupKey` and `SectionKey` with humanized labels. `ReplaceRolePermissions` validates scoped grants and replaces them transactionally. `CreateRole` creates a role with optional description. Used `SystemPermission` domain type to avoid collision with existing `domain.Permission` in `employee.go`.
